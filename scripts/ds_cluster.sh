...
#Backing up current /etc/hosts to /etc/hosts_before_ds_cluster
cp -p /etc/hosts /etc/hosts_before_ds_cluster
...
sed -i '/namenode/c\'$IP_namenode'\tnamenode' /etc/hosts 
...
sed -i '/resourcemanager/c\'$IP_resourcemanager'\tresourcemanager' /etc/hosts
...
sed -i '/hiveserver/c\'$IP_hive'\thiveserver' /etc/hosts
echo "Hive/Oozie running on $IP_hive"

#Start the IPython node
echo "Starting node1 with the IPython server..."
CID_ipython=$(docker run -d -v /root/datascience/labs:/root/labs:rw -v /root/notebooks:/root/notebooks:rw -v /root/nltk_data:/root/nltk_data --privileged --link namenode:namenode -e NODE_TYPE=workernode -e namenode_ip=$IP_namenode --dns 8.8.8.8 -p 12345:12345 -p 8888:8888 -p 22 --name node1 -h node1 -i -t hwxu/hdp_spark_node bash)
IP_ipython=$(docker inspect --format "{{ .NetworkSettings.IPAddress }}" node1)
sed -i '/node1/c\'$IP_ipython'\tnode1' /etc/hosts
...
sed -i '/'$nodename'/c\'$IP_workernode'\t'$nodename /etc/hosts
